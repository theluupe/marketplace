...
export const RESULT_PAGE_SIZE = 42;
...
/** Need a tab or category in the URL before hitting the API (matches legacy early-return). */
const shouldQueryOwnListings = queryParams =>
  !!queryParams.pub_listingType || !!queryParams.pub_categoryLevel1;
...
export const queryOwnListingsThunk = createAsyncThunk(
  ...
  {
    condition: shouldQueryOwnListings,
  }
);

// Backward compatible wrapper for the thunk
// Skip is a rejected thunk (`meta.condition`); unwrap() rejects — callers may catch.
export const queryOwnListings = queryParams => (dispatch, getState, sdk) =>
  dispatch(queryOwnListingsThunk(queryParams)).unwrap();
