...
const DraftOverlay = props => {
  const { hasImage, title, listingId, intl, inProgressListingId, onDiscardDraft } = props;
  return (
    <>
 ...
 * @param {function} [props.onClick] - TheLuupe: when provided, the thumbnail is a clickable div
 *   wired to this handler instead of upstream's built-in NamedLink. Lets the parent control
 *   navigation (e.g. body-click anywhere on the card → custom URL based on listing state).
 * @param {function} [props.onMouseOver] - TheLuupe: hover-preload hook, attached to the wrapper.
 * @param {function} [props.onTouchStart] - TheLuupe: touch-start preload hook.
...
const CardThumbnail = props => {
  ...
  const { aspectWidth = 1, aspectHeight = 1, variantPrefix: configVariantPrefix = 'listing-card' } =
    config.layout?.listingImage || {};
  const variantPrefix = variantPrefixOverride || configVariantPrefix;
  ...
  // When the parent supplies an onClick (TheLuupe's body-click pattern), suppress the inner
  // NamedLink to avoid nested click handlers fighting over navigation.
  const isLinked = !overlayType && !onClick;
  ...
  const wrapperHandlersMaybe = onClick
    ? { onClick, onMouseOver, onTouchStart, tabIndex: 0 }
    : { onMouseOver, onTouchStart };

  return (
    <div className={css.thumbnailWrapper} {...wrapperHandlersMaybe}>
