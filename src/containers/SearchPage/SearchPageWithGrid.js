...
// TheLuupe: when keyword search is enabled, augment the listing-field config with a
// custom 'location' filter that only applies to the 'creatives' category and the
// 'profile-listing' type. Used to feed both `getDerivedRenderData` and `onResetAll`
// (both consult `config.listing.listingFields`).
const buildAugmentedConfig = config => {
  const isKeywordSearch = isMainSearchTypeKeywords(config);
  if (!isKeywordSearch) return config;

  const baseFields = config?.listing?.listingFields || [];
  const creativesLocationField = {
    key: 'location',
    scope: 'public',
    schemaType: 'location',
    filterConfig: {
  ...
  return {
    ...
    listing: {
      ...config.listing,
      listingFields: [...baseFields, creativesLocationField],
    },
  };
};
...
export class SearchPageComponent extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      // TheLuupe: user-controlled grid layout; defaults to MASONRY but the creatives
      // category overrides it to SQUARE in render().
  ...
  resetAll(e) {
    ...
    onResetAll({
      history,
      routeConfiguration,
      // TheLuupe: feed the augmented config so the keyword/creatives location filter
      // is included in the reset query-param list.
      config: buildAugmentedConfig(config),
      location,
  ...
  render() {
    ...
    // TheLuupe: augment config with the keyword/creatives location filter.
    const augmentedConfig = buildAugmentedConfig(config);
    ...
    // TheLuupe: 'creatives' category forces SQUARE grid layout (and hides the toggle)
    // so creative profiles render in the gallery-style cards.
    ...
    // TheLuupe: favorites map fed to listing cards.
