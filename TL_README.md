...
| [spec/upstream-merge-v10.14-to-v11.0.2.md](./spec/upstream-merge-v10.14-to-v11.0.2.md) | 🟡 Pending | Executable plan for the in-progress merge of `original` (Sharetribe `web-template` v10.14.0 → v11.0.2) into `core-upgrade/v11.0.2-v1`. 244 commits, 27 conflicted paths, 4 major upstream refactors (CRA-eject, AuthenticationPage, ListingPage, SearchPage, ManageListingCard). All 9 load-bearing decisions resolved (single merge commit, AuthenticationPage full hooks rewrite with SSO-only invariant, γ-inverted `TheLuupeListingPageGate` for HIDDEN_PRODUCT/PORTFOLIO/PROFILE, in-merge `sanitize.js` allow-list, in-merge `getListingsById` migration for all 6 callers, take upstream `discardDraft` minus the "Finish listing" CTA). |
